...
sequencenovaccinate = [i for i in range(G.nodes*33/100)]
for x in sequencenovaccinate:
    G.nodes[i]['vacunable']=False
    G.nodes[i]['color']="green"

#Depenenet del anterior posar aleatoriament els nodes l'estat 'vacunat' depenen del % del input
sequencevaccinate = [i for i in range(G.nodes*33/100)]
for x in sequencevaccinate:
    if G.nodes[i]['color']!='green':
        G.nodes[i]['vacunat']=True
        G.nodes[i]['color']="green"

estats=[]
# posem un exemple del graph amb nomes un infectat
...
def update_graph(dia, contagirate, quarantine, novacines, vacines):
